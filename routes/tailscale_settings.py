    ...
    try:
        ...
        if status and isinstance(status, dict) and 'Self' in status:
            ...
            if isinstance(self_info, dict):
                ...
                # Update subnet routes from actual state using the proper detection method
                active_subnet_routes = TailscaleClient.subnet_routes()
                settings['advertised_routes'] = active_subnet_routes
                
                # Check for pending approval routes
                configured_routes = settings.get('advertised_routes', [])
                if isinstance(configured_routes, str):
                    configured_routes = [r.strip() for r in configured_routes.split(',') if r.strip()]
                
                # Routes that are configured but not yet active (pending approval)
                pending_routes = [route for route in configured_routes if route not in active_subnet_routes]
